...
### What NetSpectre Does:

```
User Traffic (PCAP) / Live Interface → [NetSpectre Engine] → Filtered Traffic (PCAP)
                                           ↓
                     - Identifies apps (YouTube, Facebook, etc.)
                     - Extracts SNI (Domains)
                     - Maps IP to Location (GeoIP)
                     - Fingerprints TLS Clients (JA3)
                     - Detects Anomalies (Port Scans & SYN Floods IDS)
                     - Generates real-time WebSocket Dashboard
...
### Two Versions & Dashboard

| Version                   | File                     | Use Case                                          |
| ------------------------- | ------------------------ | ------------------------------------------------- |
| Simple (Single-threaded)  | `src/main_working.cpp`   | Learning, basic PCAP analysis                     |
| Multi-threaded            | `src/dpi_mt.cpp`         | Production, large PCAPs                           |
| Live Capture              | `src/main_live.cpp`      | Real-time analysis from NIC (`en0`, `eth0`)       |
| **Interactive Dashboard** | `src/main_dashboard.cpp` | Visualizing GeoIP, JA3, Apps & IDS via WebSockets |
...
│   ├── ja3_fingerprint.h      # TLS Client Fingerprinting
│   ├── geoip.h                # IP-to-Country Mapping Database
│   ├── anomaly_detector.h     # IDS (SYN Flood & Port Scans)
...
│   ├── dashboard_server.h     # WebSocket + HTTP Server
...
│   ├── ja3_fingerprint.cpp    # JA3 hash computation
│   ├── geoip.cpp              # GeoIP resolution
│   ├── anomaly_detector.cpp   # IDS logic
│   ├── dashboard_server.cpp   # Web interface & WebSockets
│   ├── main_working.cpp       # ★ SIMPLE VERSION ★
│   ├── dpi_mt.cpp             # ★ MULTI-THREADED VERSION ★
│   ├── main_live.cpp          # ★ LIVE CAPTURE VERSION ★
│   └── main_dashboard.cpp     # ★ WEBSOCKET DASHBOARD ★
│
├── Makefile                   # For building all binaries
└── README.md                  # This file!
```
...
- **Make** installed
- **libpcap** installed (for live capture and dashboard)

### Build Commands

We use a simple `Makefile` to compile the 4 executables.

```bash
make all
```

**This generated 4 binaries in the `build/` directory:**

1. `build/dpi_engine`
2. `build/dpi_mt`
3. `build/dpi_live`
4. `build/dpi_dashboard`

### Running

**Option A - Parse a PCAP via Dashboard (Best Visuals) 🌟:**

```bash
./build/dpi_dashboard --pcap mycapture.pcap --port 8080
# => Open http://localhost:8080 to see GeoIP, JA3, Graphs, and IDS alerts!
```

**Option B - Live Capture Dashboard:**

```bash
sudo ./build/dpi_dashboard --interface en0 --port 8080
```

**Option C - Terminal Output (Simple Version):**

```bash
./build/dpi_engine test_dpi.pcap output.pcap
```

**With blocking (Dashboard example):**

```bash
./build/dpi_dashboard --pcap test_dpi.pcap \
    --block-app YouTube \
    --block-app TikTok \
